...
Function Get-TbtControllerInfo
{
    ...
    switch ($DrvType)
    {
        #https://github.com/mosdeo/N1MTF24W/tree/master/FwUpdateAPI
        "Normal"
        {
            try{
                $Controllers=[FwUpdateAPI.SdkTbtController]::GetControllersFromWmi()
                $Info=$Controllers.values | Select-Object -Property *
                [Array]$TBController=Get-PnpDevice -Class system | Where-Object {$_.Service -EQ "nhi"}
                $DrvVersion=($TBController | Get-PnpDeviceProperty -KeyName "DEVPKEY_Device_DriverVersion").Data
                $Info | Add-Member NoteProperty -Name "Driver Version" -Value $DrvVersion
            }
            catch
            {
                write-host "Error getting driver version"
        ...
        "UWP" 
        {
            $info=[TbtUWPNvm.TbtNvm]::HostInfo()
            
        }

    }
    return $Info
    } -ArgumentList $DrvType
